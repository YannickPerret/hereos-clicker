  ...
  return (
    <div className="min-h-screen bg-cyber-black flex flex-col items-center relative">
      ...
      <div className="w-full max-w-2xl relative z-10 px-4 py-12">
        <Link
          href="/login"
          className="inline-block mb-6 text-xs uppercase tracking-widest text-gray-500 hover:text-cyber-pink transition-colors"
        >
          &larr; {t('legal.back')}
        </Link>
        ...
        <h1 className="text-2xl font-bold text-cyber-pink tracking-widest text-center mb-2">
        ...
        <div className="bg-cyber-dark border border-cyber-pink/20 rounded-lg p-6 space-y-6">
          <p className="text-sm text-gray-400">{t('legal.privacy.intro')}</p>

          {sections.map((key) => (
            <div key={key}>
              <h2 className="text-xs font-bold uppercase tracking-widest text-cyber-pink mb-2">
                {t(`legal.privacy.${key}Title`)}
              </h2>
              <p className="text-sm text-gray-400">{t(`legal.privacy.${key}`)}</p>
            </div>
          ))}
        </div>
      </div>
